...
func CreateEventHandler(ctx context.Context, s source.Source, n notifier.Notifier) handlerFunc {
	return func(writer http.ResponseWriter, request *http.Request) error {
		slog.Debug("handling incoming webhook", "source", s.Key(), "path", request.URL.Path)

		notifications, err := s.Parse(request)
		if err != nil {
			slog.Error("failed to generate notifications", "source", s.Key, "error", err)
			return fmt.Errorf("failed to generate notifications: %w", err)
		}

		if len(notifications) == 0 {
			slog.Debug("no notifications to send", "source", s.Key)
